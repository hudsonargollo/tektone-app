...
export default function WizardManualQuestStep() {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
      <WizardProgress index={2} total={4} />
      <WizardSummaryChips clientId={clientId} clients={clients} assignees={assignees} priority={priority} dueDate={dueDate} />
      <Text style={styles.headline}>Como foi a jornada?</Text>
      <Text style={styles.hint}>Detalhes, novas ideias, descobertas, obstáculos técnicos pelo caminho.</Text>
      <TextInput
        autoFocus
        multiline
        value={sections.quest}
        onChangeText={(v) => setSections({ ...sections, quest: v })}
        placeholder="Descreva a jornada…"
        placeholderTextColor={colors.stone400}
        style={[styles.input, { minHeight: 160, textAlignVertical: "top" }]}
      />
      <Pressable onPress={() => router.push("/wizard/manual-victory")} style={styles.button}>
